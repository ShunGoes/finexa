...
const ResultCard = ({text, hasBorder, hasEdge, title}: iResultItems) => {
  let carded ;
  ...
  return (
    <div className={`h-[357px] w-10/12 md:w-4/12 rounded-[20px] shrink-0 overflow-hidden flex flex-col  bg-black/70  ${carded}`}>
        <div className='h-[20%] border-b-2 w-full border-b-[#FCF7FC] bg-[#783888] flex justify-center items-center'>
            <h4 className="text-[20px]   leading-[20px] font-[500] text-[#FFFFFF]">{title}</h4>
        </div>
        <div className='h-[80%]  p-[20px] flex justify-center items-center'>
            <p className='font-[400] text-[16px] leading-[20px] text-[#FCF7FC]'>{text}.</p>
        </div>
    </div>
  )
}
